...
void Backend::sendOrderEvent(hello_TaxiEvent event, ITaxiOrder *order)
{
	...
	switch (event) {
		...
		case hello_TaxiEvent_END_CLIENT_MOVE:
			/*
			ты мне должен будешь передать еще р€д параметров, а именно
			1. кол-во минут движени€ с багажом
			2. кол-во минут остановок по просьбе клиента
			3. кол-во минут остановок по просьбе водител€
			4. сумма обща€ - уже есть
			5. сумма по межгороду
			6. отдельно сумма по городу
			7. врем€ поездки - кол-во минут
			8. километраж по межгороду в одну сторону
			9. километраж по межгороду в другую сторону (пока еще не определились с этим) - поэтому передвай пока весь километраж по межгороду в пункте 8, сюда пока пиши 0, как разберемс€ - будешь правильную цифру писать
			10. километржа по городу
				возможно по€витс€ что-то еще... эти все данные нужны дл€ статистики, дл€ разборок с клиентами и дл€ статистики
...
// сообщение не будет доставлено, если нет св€зи
